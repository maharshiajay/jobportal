...
$(document).on('ready',function(){ 
	...
	/*jobtesti script code start here*/
	$(".jobtesti").owlCarousel({
		 items : 2,
         itemsDesktop : [1199, 2],
         itemsDesktopSmall : [979, 2],
         itemsTablet : [768, 2],
    ...
    // Product List
	$('#list-view').on('click',function(){
		$('.row .product-grid').attr('class', 'product-list col-xs-12');
		localStorage.setItem('display', 'list');
	});
	
	// Product Grid
	$('#grid-view').on('click',function(){
		$('.row .product-list').attr('class', 'product-grid col-lg-3 col-md-3 col-sm-6 col-xs-12');
		localStorage.setItem('display', 'grid');
	});
    ...
    $('.tab-part a[href^="."]').on('click', function(event) {
